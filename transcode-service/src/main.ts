...
function shouldIgnore(name: string) {
  if (name.startsWith("messages/") || name.startsWith("posts/messages/")) return true;
...
function build720VariantPath(objectName: string) {
...
async function updatePostVariants(
  ...
  const { error } = await supabase
    ...
    .or(
      `media_path.eq.${sourceName},media_path.eq.${objectName},media_url.eq.${sourceName},media_url.eq.${objectName}`,
    );
  if (error) throw error;
}
